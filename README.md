...
빈 생명주기 콜백
 - 애플리케이션 작 시점에 필요한 연결을 미리 해두고, 애플리케이션 종료 시점에 연결을 모두 종료하는 작업을 진행하기 위해서는 객체의 초기화 종료 작업이 필요하다.
 - 스프링은 초기화 작업과 종료 작업 진행을 위한 콜백을 제공해준다.
 - 스프링은 의존관계 주입이 완료되면 스프링 빈에게 콜백 메서드를 통해서 초기화 시점을 알려주는 다양한 기능 제공
 - 스프링 컨테이너가 종료되기 직전에 소멸 콜백을 줌.
 - 스프링 빈의 이벤트 라이프사이클
   - 스프링 컨테이너 생성 -> 스프링 빈 생성 -> 의존관계 주입 -> 초기화 콜백 -> 사용 -> 소멸전 콜백 -> 스 프링 종료
...
빈 스코프
 - 스프링 빈은 스프링 컨테이너의 시작과 함께 생성되어서 스프링 컨테이너가 종료될 때 까지 유지한다.
 - 스프링빈이 기본적으로 싱글톤 스코프로 생성되기 때문인데, 스코프는 번역 그대로 빈이 존재할 수 있는 범위를 뜻한다.
...
프로토타입
 ...
 - 이후 스프링 컨테이너는 프로토타입 빈을 관리하지 않고, 프로토타입 빈을 관리할 책임은 프로토타입 빈을 받은 클라이언트에 있다. -> @PreDestory 종료 메서드가 호출 안됨.

   ![캡처](https://user-images.githubusercontent.com/62634760/106414844-fac1b880-6490-11eb-9c50-f3f4bb88f25f.PNG)



프로토타입을 혼자 사용하는 건 괜찮지만 싱글톤빈과 함께사용할때 문제 발생
 - 싱글톤에서 프로토타입 빈을 사용할 때 문제점이 발생한다.
 - 예를 들어 ClientBean(싱글톤)이 PrototypeBean(프로토타입)을 불러쓰는 상황에서, 싱글톤은 스프링 컨테이너 생성 시점에 함께 생성되고, 의존관계 주입도 발생하기 때문에 ClientBean은 PrototypeBean(프로토타입)을 한번 주입받고 주입받은 객체를 계속 유지하게 된다.
 - 결국 프로토타입을 사용하는 의미가 없게 된다.

 - 그래서 프로토타입 스코프 -> 싱글톤 빈과 함께 사용시 Provider로 문제 해결
